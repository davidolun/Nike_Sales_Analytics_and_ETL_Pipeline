...
class ETLConfig:
    ...
    DB_PATH = str(DATA_DIR / "nike_sales.db")
    TABLE_NAME = "nike_sales_cleaned"
...
# Loading
# --------------------------
def load_data(df, csv_path=None, db_path=None, table_name=None):
    """
    ...
    Args:
        ...
        table_name (str, optional): Name of the database table
    """
    logging.info("\n" + "="*70)
    ...
    try:
        ...
        if db_path and table_name:
            logging.info(f"Loading data to database: {db_path}")
            ...
            logging.info(f"✅ Loaded {count:,} records to table '{table_name}'")
            
            # Create indexes for better query performance
            cursor.execute(f"CREATE INDEX IF NOT EXISTS idx_order_date ON {table_name}(Order_Date)")
            cursor.execute(f"CREATE INDEX IF NOT EXISTS idx_region ON {table_name}(Region)")
            logging.info(f"✅ Created indexes on Order_Date and Region")
...
def main():
    ...
    try:
        ...
        load_data(
            ...
            db_path=ETLConfig.DB_PATH,
            table_name=ETLConfig.TABLE_NAME
        )
